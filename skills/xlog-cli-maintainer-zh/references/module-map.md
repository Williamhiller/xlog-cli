...
- `bin/xlog-cli.js`: CLI 路由，处理 `serve`、`query`、`sessions`、`bugpack`、`mcp`。
- `bin/xlog-mcp.js`: MCP stdio server 入口。
- `src/index.js`: 包导出面和兼容别名。

## Runtime 采集层

- `src/runtime/interceptor.js`: 浏览器/runtime 拦截、参数序列化、capture 状态、失败重试缓冲与上报。
...
- `src/server/server.js`: HTTP API、SSE、viewer 静态资源分发。
...
- `src/plugins/vite-plugin.js`: Vite 注入逻辑和进程内 xlog server 启动。
- `src/plugins/webpack-plugin.js`: Webpack 注入逻辑和进程内 xlog server 启动。
